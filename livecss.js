...
var livecss = {
  ...
  watchAll: function() {
    ...
    //got rid of notifications every time it runs, but it's good to know if it's running at all
    console.log("livecss started and is watching All")
  },
  ...
  replaceLinkElement: function(linkElement, stylesheetUrl) {
    ...
    //PATCH BY ELI
    //Make livecss behave when javascript reloads. not sure why this would bust
    //but it would make livecss not work until a page reload. bummer.
    try {
      if (sibling)
...
if (window.location.search.toString().indexOf("startlivecss") >= 0)
  livecss.watchAll();